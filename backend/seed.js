...
const getCategoryImagePrompt = (category) => {
  const map = {
...
const generateMockPosts = () => {
  ...
  categories.forEach((category) => {
    // Generate 5 posts per category for initial seed
    for (let i = 0; i < 5; i++) {
...
const seedDB = async () => {
  try {
    ...
    console.log('🌱 Generating and seeding new posts...');
    ...
    if (err.code === 8000 || (err.message && err.message.includes('bad auth'))) {
      ...
      console.error('2. Did you copy the password correctly? (Try typing it manually to avoid spaces)');
